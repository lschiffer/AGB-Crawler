...
def getPermissions(ids):
    ...
    content = r.text
    pattern  ='\[\[\"(?P<permission>.{0,50})\",\"Erm'   #\".\{0,400\}\",[0|1]\]"
    result = re.findall(pattern,content)
    if result:
    #resolve redirect
        #print (result)
        #sys.stdout.buffer.write(result)
        #print (result.group('permission'))
...
for row in id_list:
    ...
    #conn.execute("Update agb_links SET permissions='"+permissions.decode('utf-8')+"' WHERE id = '"+row[0]+"';")
